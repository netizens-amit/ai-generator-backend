// src/generation/services/enhanced-prompt-builder.service.ts
...
import { DESIGN_SYSTEMS } from '../prompts/design-systems';

@Injectable()
export class EnhancedPromptBuilderService {
  private logger = new Logger(EnhancedPromptBuilderService.name);

  /**
   * Build enhanced prompt inspired by Open Lovable
   */
  async buildEnhancedPrompt(preferences: GenerateWebsiteDto): Promise<string> {
   ...
   */
  private buildReactPrompt(prefs: GenerateWebsiteDto): string {
...
Generate a PERFECT, PRODUCTION-READY ${industry.name} website for ${prefs.companyName} NOW!`;
   ...
   * Build HTML prompt
   */
  private buildHTMLPrompt(prefs: GenerateWebsiteDto): string {
    const industry = INDUSTRY_TEMPLATES[prefs.industry] || INDUSTRY_TEMPLATES['business'];
    const design = DESIGN_SYSTEMS[prefs.designStyle] || DESIGN_SYSTEMS['modern'];
